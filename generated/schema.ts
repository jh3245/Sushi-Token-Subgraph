...
export class Whale extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save Whale entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type Whale must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("Whale", id.toString(), this);
    }
  }

  static loadInBlock(id: string): Whale | null {
    return changetype<Whale | null>(store.get_in_block("Whale", id));
  }

  static load(id: string): Whale | null {
    return changetype<Whale | null>(store.get("Whale", id));
  }
